...
(3, 'Interstellar', 'A team of explorers travel through a wormhole in space in an attempt to ensure humanity\'s survival.', 'English', 169);

INSERT IGNORE INTO show_timings (id, movie_id, show_time, screen_name) VALUES 
(1, 1, DATE_ADD(CURRENT_DATE, INTERVAL 1 DAY) + INTERVAL 10 HOUR, 'Screen 1'),
(2, 1, DATE_ADD(CURRENT_DATE, INTERVAL 1 DAY) + INTERVAL 14 HOUR, 'Screen 1'),
(3, 2, DATE_ADD(CURRENT_DATE, INTERVAL 1 DAY) + INTERVAL 18 HOUR, 'Screen 2'),
(4, 3, DATE_ADD(CURRENT_DATE, INTERVAL 2 DAY) + INTERVAL 20 HOUR, 'Screen 3');

-- Insert seats for show 1, 2, 3, 4 (A1 to A5, B1 to B5)
INSERT IGNORE INTO seats (show_id, seat_number, is_booked) 
SELECT st.id, CONCAT(r.row_name, n.num), FALSE
FROM show_timings st
CROSS JOIN (SELECT 'A' as row_name UNION SELECT 'B' as row_name) r
